    ...
    <div>
      ...
      <div className="space-y-2.5">
        {factors.map((f) => {
          ...
          const isAiEstimated = sourceLabel === 'ai_estimated'
          return (
            <div key={f.labelKey}>
                ...
                <div className="flex-1 h-2 rounded-full bg-primary-50 overflow-hidden flex items-center">
                  <motion.div
                    ...
                    className={`h-full rounded-full ${
                      f.isBaseline ? 'bg-primary-400' : isAiEstimated ? 'bg-purple-400' : positive ? 'bg-teal-500' : 'bg-red-400'
                    }`}
                  />
                </div>
                <span
                  className={`w-10 shrink-0 text-right text-[12px] font-bold ${
                    f.isBaseline ? 'text-primary-700' : isAiEstimated ? 'text-purple-700' : positive ? 'text-teal-700' : 'text-red-600'
                  }`}
                >
              ...
              {sourceLabel && sourceLabel !== 'baseline' && (
                <p
                  className={`ml-[172px] mt-0.5 flex items-start gap-1 text-[10px] leading-snug ${
                    isAiEstimated ? 'text-purple-700/70' : 'text-ink-900/40'
                  }`}
                >
                  {isAiEstimated && <Sparkles size={10} className="shrink-0 mt-0.5" />}
                  <span>
                    {SOURCE_NOTE_KEYS[sourceLabel] ? t(SOURCE_NOTE_KEYS[sourceLabel]) : sourceLabel}
                    {f.source?.asOf ? ` · ${f.source.asOf}` : ''}
                    {isAiEstimated && f.note ? ` — "${f.note}"` : ''}
                  </span>
                </p>
              )}
            </div>
